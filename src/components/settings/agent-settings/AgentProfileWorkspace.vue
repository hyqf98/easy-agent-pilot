...
watch(
  ...
  async (agentId) => {
    ...
    await Promise.all([
      ...
      agentStudioStore.agents.length === 0 ? agentStudioStore.loadAgents() : Promise.resolve(),
...
const teamMemberships = computed(() => {
  ...
  const studioAgentIds = agentStudioStore
    .getByRuntimeAgentId(props.agent.id)
    .map(agent => agent.id)

  if (studioAgentIds.length === 0) {
    return []
  }

  return agentTeamStore.teams
    .filter(team => team.members.some(member => studioAgentIds.includes(member.agentId)))
    .map(team => ({
      team,
      membership: team.members.find(member => studioAgentIds.includes(member.agentId)) || null
    }))
})
