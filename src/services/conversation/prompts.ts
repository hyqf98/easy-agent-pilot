...
2. Output exactly one <form-request> XML tag containing a valid JSON object. Do NOT wrap it in a code block.
3. The JSON object must use type="form_request" and include a short question plus exactly one formSchema object.
...
10. When outputting form_request, do not add explanations, headings, greetings, bullet points, or any extra text before or after the <form-request> tag.
...
17. Every field MUST include a "suggestion" value — your recommended default answer based on your analysis. This is shown as a pre-filled hint so the user can accept it or change it.
18. For select, radio, and multiselect fields, you MUST set "allowOther": true and "otherLabel": "Other (custom)" so the user can type their own answer when none of the provided options fit.

Valid shape example:
<form-request>
{"type":"form_request","question":"Please confirm the missing parameters","formSchema":{"formId":"deployment","title":"Deployment","fields":[{"name":"interface","label":"Interface","type":"radio","suggestion":"cli","allowOther":true,"otherLabel":"Other (custom)","options":[{"label":"CLI","value":"cli"},{"label":"Web","value":"web"}]},{"name":"region","label":"Region","type":"select","suggestion":"us-east-1","allowOther":true,"otherLabel":"Other (custom)","options":[{"label":"US East","value":"us-east-1"},{"label":"EU West","value":"eu-west-1"}]}]}}
</form-request>`
  }
...
2. 只输出一个 <form-request> XML 标签，内含合法 JSON 对象，且不要放在代码块里。
...
10. 输出 form_request 时，<form-request> 标签前后不要再附加解释、标题、问候语、列表或其他任何文本。
...
17. 每个字段必须填写 suggestion（你的推荐默认值），用于回显你的建议，让用户可以直接采纳或修改。
18. select、radio、multiselect 字段必须设置 "allowOther": true 和 "otherLabel": "其他（自定义）"，让用户在选项都不合适时可以自己输入。

合法结构示例：
<form-request>
{"type":"form_request","question":"请确认缺失参数","formSchema":{"formId":"deployment","title":"部署形态","fields":[{"name":"interface","label":"交互形式","type":"radio","suggestion":"cli","allowOther":true,"otherLabel":"其他（自定义）","options":[{"label":"CLI 命令行","value":"cli"},{"label":"Web 应用","value":"web"}]},{"name":"region","label":"部署区域","type":"select","suggestion":"cn-east","allowOther":true,"otherLabel":"其他（自定义）","options":[{"label":"华东","value":"cn-east"},{"label":"华北","value":"cn-north"}]}]}}
</form-request>`
}
