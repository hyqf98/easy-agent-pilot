/**
 * useSheetEditor — Sheet (xlsx) 编辑器的全部业务逻辑。
 *
 * 职责：
 * 1. 基于 useUniverEngine 初始化 Univer Sheets 实例并挂载到容器；
 * 2. 把 Uint8Array 形态的 xlsx 缓冲转换为 Univer 快照（bufferToUniverSnapshot），
 *    反向把 Univer 工作簿快照导出为 xlsx Uint8Array（getUniverSnapshot）；
 * 3. 缓存当前快照到 store，并向 store 注册导出 provider，供外部保存流程调用；
 * 4. 监听 props.buffer 变化自动重新加载，组件卸载时清理引擎并回写缓存。
 *
 * 该 composable 不直接渲染模板，容器 ref 通过返回值暴露给模板使用。
 */
import { ref, onMounted, onBeforeUnmount, watch } from 'vue'
import { useUniverEngine } from '../../composables/useUniverEngine'
import {
  useOfficeViewerStore,
  setCachedSnapshot,
  getCachedSnapshot
} from '../../stores/officeViewer'
...
/** SheetEditor 组件 Emits */
export interface SheetEditorEmits {
  (e: 'loading', value: boolean): void
}

export function useSheetEditor(
  props: SheetEditorProps,
  emit: SheetEditorEmits
) {
