/**
 * 职责：任务拆分（Plan Split）会话的响应式 store。
 *
 * 仅负责状态、生命周期与流式事件编排；所有无副作用的纯函数 helper、接口与模块级常量
 * 已抽离至 ./taskSplitShared（与 taskExecution + taskExecutionShared 范式对齐），通过下方
 * `import { ... } from './taskSplitShared'` 引用。
 *
 * 仍保留在本文件的模块级可变状态（streamBuffer / 各类定时器 / 异常计数），因其需被 store
 * 内部函数反复重新赋值、且与响应式状态强耦合，ES Module 导入绑定无法被重新赋值，故不外迁。
 *
 * 向后兼容：以下符号被单测/外部直接引用，通过 re-export 保持 '@/stores/taskSplit' 入口不变。
 */
...
import { normalizeFormSchemaForRendering } from '@/utils/formSchema'
import { resolveSubAgentById } from '@/services/subAgent/runtime'
import {
...
import {
  ...
  // —— 纯函数 helper（日志解析 / 结构化恢复 / 请求构造 / prompt 装配 / 性能度量） ——
  isAnomalousContent,
  clampLogContent,
  measurePlanSplit,
...
} from './taskSplitShared'
import type {
  TaskSplitContext,
  TaskSplitRefinementState,
  ContinueSessionWithInstructionOptions,
  SubmittedFormSnapshot,
  PlanSplitRuntimeMetrics,
  RustPlanSplitLogRecord
} from './taskSplitShared'

// 保持调用方/单测零改动：下列符号仍可从 '@/stores/taskSplit' 直接引用
export { isAnomalousContent, clampLogContent, PLAN_SPLIT_STREAMING_LIMITS } from './taskSplitShared'

// —— 流式运行期可变单例状态 ——
// 说明：以下 let 变量需被 store 内部函数反复重新赋值（flush / 重置 / 熔断），
// 而 ES Module 的导入绑定只读、不可重新赋值，故保留在主 store 文件内、未迁入 taskSplitShared。
