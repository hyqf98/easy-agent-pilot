...
export function usePlanSplitConversation() {
  ...
  const splitChatMessages = computed<Message[]>(() => {
    ...
    turns.forEach((turn, turnIndex) => {
      ...
      // 一事件一行 + seq 排序（对齐主会话模型）
      // 共享模块 buildTurnAssistantMessages 处理 tool_use/thinking/content/tool_result 等独立成行
  ...
  /**
   * 停止拆分并关闭会话与任务 Dock（取消场景）。
   * 操作栏的「停止」按钮入口：停止 AI 运行 → 关闭拆分会话 → 关闭右侧任务 Dock。
   */
  async function stopAndClose() {
    ...
    await closeDialog()
    // 关闭右侧任务 Dock（取消拆分无持久化任务可展示）
    const { useRightTaskPanelStore } = await import('@/stores/rightTaskPanel')
    await useRightTaskPanelStore().close()
  }
