...
pub fn init_database() -> Result<()> {
    ...
    let agent_profiles_table_sql = r#"
        CREATE TABLE IF NOT EXISTS agent_profiles (
            agent_id TEXT PRIMARY KEY,
            system_prompt TEXT,
            role_definition TEXT,
    ...
    "#;
    if let Err(e) = conn.execute(agent_profiles_table_sql, []) {
        println!("Agent profiles table migration warning: {}", e);
    }

    let agent_profiles_indexes = [
        "CREATE INDEX IF NOT EXISTS idx_agent_profiles_updated ON agent_profiles(updated_at DESC)",
    ];
    for migration in agent_profiles_indexes {
        if let Err(e) = conn.execute(migration, []) {
            println!("Agent profiles index migration warning: {}", e);
        }
    }
